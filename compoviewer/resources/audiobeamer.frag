...
void main(void)
{
	...
	uv+=0.5;
	r+=0.5;
	float pos = sqrt(r2.x*r2.x+r2.y*r2.y)*0.5-texture2D(iChannel1, vec2(0.1,0.0),8.0).r*0.12;
	r2.x = iResolution.x/iResolution.y-uv.x-0.3*iResolution.y/iResolution.x;
	float pos2 = sqrt(r2.x*r2.x+r2.y*r2.y)*0.5-texture2D(iChannel1, vec2(0.1,1.0),8.0).r*0.12;
	...
	p = (gl_FragCoord.xy*vec2(1.0,-1.0) + vec2(-iResolution.x*0.45,iResolution.y*0.8) - iResolution/2.0 + logores/2.0)/logores;
